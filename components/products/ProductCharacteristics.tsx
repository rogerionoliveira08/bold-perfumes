  ...
  return (
    <section className="overflow-hidden rounded-[28px] border border-zinc-800 bg-zinc-950 p-5 shadow-[0_24px_70px_rgba(0,0,0,0.22)] sm:p-8">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
        <article className="rounded-2xl border border-zinc-800 bg-black p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {caracteristicas.map((caracteristica) => (
              <div
                key={caracteristica}
                className="group flex min-w-0 items-start gap-3 rounded-xl border border-zinc-800 bg-zinc-950 p-3.5 transition duration-300 hover:border-zinc-600 hover:bg-zinc-900"
              >
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-zinc-600 bg-white text-[9px] text-black">
                  <FaCheck />
                </span>

                <p className="text-xs font-semibold leading-5 text-zinc-200 sm:text-sm sm:leading-6">
                  {caracteristica}
                </p>
              </div>
            ))}
        ...
        <article className="rounded-2xl border border-zinc-700 bg-zinc-900 p-5 sm:p-6">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white text-black">
            <FaShieldAlt size={15} />
          </div>

          <p className="mt-5 text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">
...
function HighlightCard({
  ...
  return (
    <article className="group rounded-2xl border border-zinc-800 bg-black p-4 transition duration-300 hover:-translate-y-0.5 hover:border-zinc-600 hover:bg-zinc-900 sm:p-5">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-700 bg-white text-sm text-black transition duration-300 group-hover:scale-105">
        {icon}
      </div>

      <p className="mt-4 text-[9px] font-black uppercase tracking-[0.16em] text-zinc-400">
        {titulo}
      </p>
...
function TrustItem({ texto }: { texto: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white text-[8px] text-black">
        <FaCheck />
      </span>

      <span className="text-xs font-semibold leading-5 text-zinc-200">
        {texto}
      </span>
    </div>
  );
